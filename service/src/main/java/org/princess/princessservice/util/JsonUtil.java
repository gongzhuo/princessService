...
package org.princess.princessservice.util;
...
public final class JsonUtil {
  ...
  public static <T> T fromJson(String jsonStr, TypeReference<T> typeRef) {
    try {
      ...
      throw new IllegalArgumentException(
          "Parser json to object error, json = " + jsonStr + ", expect type = " + typeRef.getType(),
          ex);
    }
  }
  ...
  public static <T> T fromJson(File file, Class<T> objClass) {
    try {
      ...
      LOGGER.error("Parser to object error.", ex);
      throw new IllegalArgumentException(
          "Parser json to object error, file = " + file.getName() + ", expect class = " + objClass,
          ex);
    }
  }
}
